.PHONY: install install-dev lint test test-cov run ci docker-build docker-up docker-test clean
...
test-cov:
	python -m pytest tests/ -v --tb=short --cov=app --cov=config --cov-report=term-missing --cov-report=xml --cov-fail-under=80
